...
# load data needed
################################################################
imp_rough_df <- readRDS("./imp_vol_suf_results/imp_vol_surf_results.rds")
...
df_processed <- df %>% mutate(ATM_IV = rowMeans(cbind(ATMC_imp_vol,ATMP_imp_vol), na.rm=T)) %>% 
  group_by(year, month, PERMNO) %>% arrange(PERMNO, date) %>% summarise(ATM_skew = last(ATM_skew),
                                                  imp_vol_slope = last(imp_vol_slope),
                                              ATM_IV= last(ATM_IV),
                                              ATM_skew_slope = last(ATM_skew_slope)) %>% ungroup
...
df_final <- df_processed %>% 
  mutate(yearmonth = year*100 + month) %>%  filter(exchcd %in% c(1,2,3)) %>% select(yearmonth, PERMNO, ATM_skew, ATM_IV, ATM_skew_slope,
                                                                                    imp_vol_slope,retadj.nextmonth) %>% drop_na()
# Xie table 4.1
sapply(df_final  %>% filter(yearmonth<=201312)%>% select(-c(yearmonth, PERMNO)), function(x) {
...
write.csv(df_final, "Xie_final_df.csv", row.names = F)

...
# sge_run --grid_mem=15g --grid_submit=batch "./imp-vol-related-factor.R $y $m"
...
#crsp_daily <- readRDS("F:/Dropbox/AssetPricingPy/data/CRSP_data/crsp_daily.rds")
...
imp_vol_skew <- imp_vol_suf %>% filter(days %in% c(30,60)) %>% group_by(secid, date, days) %>%  
  summarize(ATM_skew = cal_avg_skew(impl_volatility, days, delta, cp_flag), ATMC_imp_vol = impl_volatility[delta==50]) %>% ungroup

imp_vol_XZZ_skew <- imp_vol_suf %>% filter(days ==30) %>% group_by(secid, date) %>%  
  summarize(XZZ_skew = impl_volatility[(delta==-25)] - impl_volatility[(delta==50)]) %>% ungroup

imp_vol_slope <- imp_vol_suf %>% filter(days %in% c(30,60)) %>% group_by(secid, date) %>%  
  summarize(imp_vol_slope = cal_avg_vol_slope(impl_volatility, days, delta, cp_flag)) %>% ungroup
imp_vol_skew_slope <- imp_vol_skew %>% group_by(secid, date) %>% summarise(ATM_skew_slope = ATM_skew[days==60] - ATM_skew[days==30]) %>% ungroup
...
ATM_skew_P <- option_price_CRSP_merged %>% filter(cp_flag == "P") %>% drop_na() %>% distinct(date,PERMNO,exdate,strike_price,.keep_all = T) %>% group_by(date, PERMNO, exdate) %>%
  summarize(nonpara_ATM_skew = ATMskew_term_structure_spline(logKS, impl_volatility,3), tau = first(tau), secid = first(secid), tot_option_VOL = sum(volume), tot_open_interest=sum(open_interest))
...
P_imp_rough <- ATM_skew_P %>% drop_na %>% summarize(tot_option_VOL = sum(tot_option_VOL), tot_open_interest=sum(tot_open_interest),
                                                                alpha = alpha_estimation(tau, nonpara_ATM_skew,1), alpha_rsquare = alpha_estimation_rsquare(tau, nonpara_ATM_skew,1), 
                                                                num_ATM_skew = n(), secid = first(secid)) %>% ungroup()

C_imp_rough <- ATM_skew_C %>% drop_na %>% summarize(tot_option_VOL = sum(tot_option_VOL), tot_open_interest=sum(tot_open_interest),
                                                    alpha = alpha_estimation(tau, nonpara_ATM_skew,1), alpha_rsquare = alpha_estimation_rsquare(tau, nonpara_ATM_skew,1), 
                                                    num_ATM_skew = n(), secid = first(secid)) %>% ungroup()
...
saveRDS(df_to_save, paste("./imp_rough_results/", year_, "_",month_, "_imp_rough.rds", sep=""))


#######################################################################################################################
# prepare for potential ML, preprocess imp vol surface data by months
#######################################################################################################################
imp_vol_suf <- imp_vol_suf %>% select(date, secid, delta,impl_volatility, days) %>% filter(days <= 200)
